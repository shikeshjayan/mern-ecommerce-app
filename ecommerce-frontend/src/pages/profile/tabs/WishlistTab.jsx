...
const WishlistTab = ({ wishlistItems, addedAllToCart, onAddAll, onRemove, onAddToCart }) => (
  <div className="space-y-6">
      ...
      {wishlistItems.length > 0 && (
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-black transition-all active:scale-95 ${
            addedAllToCart ? "bg-green-600 text-white" : "bg-gray-900 hover:bg-black text-white"
          }`}
        >
    ...
    {wishlistItems.length === 0 ? (
      <div className="flex flex-col items-center py-24 gap-4 bg-white rounded-2xl border-2 border-dashed border-gray-200">
        ...
        <h4 className="text-lg font-black text-gray-800">Your wishlist is empty</h4>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {wishlistItems.map((item) => (
          <WishlistCard key={item._id || item.id} item={item} onRemove={onRemove} onAddToCart={onAddToCart} />
        ))}
      </div>
    )}
  </div>
