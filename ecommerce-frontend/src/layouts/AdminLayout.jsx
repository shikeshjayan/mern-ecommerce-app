...
const AdminLayout = () => {
    ...
    const searchProducts = async () => {
      ...
      try {
        const res = await apiClient.get(
          `/api/v1/products?keyword=${searchQuery}&limit=5`,
        );
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 flex font-sans transition-colors duration-300">
      ...
      <div
        className={`
        fixed lg:static inset-y-0 left-0 z-40 w-64 bg-[#0a0a0a] dark:bg-slate-950 text-gray-300 flex flex-col shadow-2xl transform transition-transform duration-300
      ...
      >
        ...
        {/* Bottom - User & Logout */}
        <div className="p-4 border-t border-gray-800/50 m-4 bg-gray-900/50 dark:bg-slate-900/50 rounded-2xl">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Top Navbar */}
        <header className="h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-200 dark:border-slate-800 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-10 shadow-sm transition-colors duration-300">
          <button
            onClick={() => setSidebarOpen(true)}
            className="lg:hidden p-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-white"
          >
          ...
          <div
            className="relative flex-1 max-w-md mx-4 lg:mx-0 lg:ml-0"
            ref={searchRef}
          >
            <div className="flex items-center bg-gray-100 dark:bg-slate-800 px-4 py-2 rounded-full">
              ...
              <input
                ...
                className="bg-transparent border-none outline-none w-full text-sm text-gray-700 dark:text-gray-200 placeholder-gray-400"
              />
            </div>

            {/* Search Results Dropdown */}
            {showSearch && searchResults.length > 0 && (
              <div className="absolute top-full mt-2 w-full bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-gray-100 dark:border-slate-800 overflow-hidden z-50">
                ...
                <Link
                  ...
                  className="block p-3 text-center text-sm text-orange-600 hover:bg-gray-50 dark:hover:bg-slate-800 border-t dark:border-slate-800"
                >
                  View all results
                </Link>
              </div>
        ...
        {/* Page Content */}
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 bg-[#f8fafc] dark:bg-slate-950 transition-colors duration-300">
          <Outlet />
        </main>
      </div>
